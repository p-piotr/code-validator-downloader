...
function valid_serial_code($serial_code, $shared_package_id, $cpe)
{
    ...
    $output = '';
    $sql = "SELECT products_included FROM $table_name_packages WHERE package_id = $shared_package_id;";
    ...
    {
        $product = $products[$i];
        $dynamic_path = product_download_link_exists($serial_code, $product->product_id);
        if ($dynamic_path == null)
            $dynamic_path = add_product_download_link($serial_code, $product->product_id);
